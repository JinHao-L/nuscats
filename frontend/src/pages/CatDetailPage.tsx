...
const CatDetailPage: React.FC<CatDetailsProps> = ({ cat }) => {
    ...
    cat = cat ? cat : makeCat({
        id: 1,
        name: "Sourplum",
        description: "I am a sweet kitty and I warm up to people once I spend some time with them.\n\n"
...
const CatAbout: React.FC<CatAboutProps> = ({ one_liner, description }) => {
    return <div className="flex flex-col space-y-4 overflow-y-auto h-cat-profile-content px-7">
        <h2 className="px-1 py-2 italic font-semibold text-center bg-gray-200 rounded shadow-sm text-primary-400">{one_liner}</h2>
        <p className="px-5 text-sm text-justify text-gray-700 whitespace-pre-wrap">{description}</p>
    </div>
}

type CatLocationProps = Pick<Cat, "sightings">
    ...
    return sightings
        ? <div className="flex flex-col items-center justify-start w-full space-y-3 overflow-auto px-7 h-cat-profile-content">
            {sightings.map((sighting, idx) => {
                const bgColor = sighting.type === SightingType.Emergency ? "bg-red-100" : "bg-secondary-100"
                return <div
                    ...
                    <div className="flex flex-col justify-start w-full ml-4">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <p className="text-sm text-gray-800">
                                {new Intl.DateTimeFormat('en-GB', ConvenientDateTimeFormatOptions).format(sighting.created_at)}
                            </p>
                        </div>
                        <div className="flex items-center space-x-2">
                            <IonIcon color="secondary" icon={locationOutline} />
                            {/* TODO: resolve to actual location */}
                            <p className="text-sm text-gray-800">{sighting.location.coordinates}</p>
                        </div>
